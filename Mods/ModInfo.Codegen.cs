...
namespace CodeName.Modding.Mods
{
    public partial class ModInfo
    {
        ...
        private void GenerateClass()
        {
            ...
            var overrides = new Dictionary<string, List<KeyValuePair<string, Object>>>();
            ...
            using (writer.WithCBlock($"public static partial class {Id}"))
            {
                ...
                {
                    foreach (var resource in RawResources.OrderBy(x => x.Key).ToList())
                    {
                        ...
                        if (modId != Id)
                        {
                            ...
                            {
                                overridenResources = new List<KeyValuePair<string, Object>>();
                        ...
                        WriteResourceKeyProperty(writer, resource.Key, resource.Value, modId);
                    }
                }
                ...
                using (writer.WithCBlock("public static partial class Overrides"))
                {
                    ...
                    foreach (var (modId, overridenResources) in overrides)
                    {
                        ...
                        using (writer.WithCBlock($"public static partial class {modId}"))
                        {
                            ...
                            {
                                WriteResourceKeyProperty(writer, overridenResource.Key, overridenResource.Value, modId);
                            }
                        }
                    }
                }
        ...
        private void WriteResourceKeyProperty(CodegenTextWriter writer, string key, Object asset, string modId)
        {
            var propertyName = key.Substring(modId.Length);
            propertyName = ResourceKey.Regexes.ReplaceCsharpUnsafeCharacters.Replace(propertyName, "_").Trim('_');

            writer.WriteLine($"public static global::{typeof(PropertyDefinition)}<global::{asset.GetType()}> {propertyName} {{ get; }} = new(\"{key}\");");
        }
